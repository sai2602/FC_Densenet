...
from helpers import BatchDatasetReader, get_augmented_data
import numpy as np
from common.CsvReader import CsvReader
from common.DatasetReader import normalize_meanstd
import math
import time
...
class DenseTiramisu(object):
    ...
    def train(self, train_path, val_path, save_dir, batch_size, epochs, learning_rate):
        ...
        Returns:
            None
        """
        train_image = 'train_data_list_new_data.txt'
        val_image = 'test_data_list_update.txt'
        ...
        train_dataset_reader = BatchDatasetReader(train_image_paths, image_size=(800, 800))
        test_dataset_reader = BatchDatasetReader(val_image_paths, image_size=(800, 800))
        ...
        loss_summary = tf.summary.scalar("BCE", loss)
        ...
        with tf.Session() as sess:
            ...
            path_1 = '/data/Packaging_detection/Raw/Model_Logs_packaging/train_text/train_loss.txt'
            path_2 = '/data/Packaging_detection/Raw/Model_Logs_packaging/val_text/val_loss.txt'
            path_3 = '/data/Packaging_detection/Raw/Model_Logs_packaging/iou_text/train_iou.txt'

            path_1_handle = open(path_1, 'w+')
            path_2_handle = open(path_2, 'w+')
            path_3_handle = open(path_3, 'w+')

            train_writer = tf.summary.FileWriter('/data/Packaging_detection/Raw/Model_Logs_packaging/train', sess.graph)
            iou_writer = tf.summary.FileWriter('/data/Packaging_detection/Raw/Model_Logs_packaging/iou', sess.graph)
            validation_writer = tf.summary.FileWriter('/data/Packaging_detection/Raw/Model_Logs_packaging/validation')

            # trained_model = "/data/Packaging_detection/Raw/Model_Logs_packaging/model.ckpt-24"
            # saver.restore(sess, trained_model)
            # print("Model restored...", trained_model)
            validation_cost = 5000
            step = 0  # should be made 0
            ...
            for epoch in range(epochs):
                ...
                for train_step in range(self.num_train_images // batch_size):
                    ...
                    image_aug_batch, mask_aug_batch = get_augmented_data(image_batch, mask_batch)
                    image_batch = normalize_meanstd(image_batch, axis=(1, 2))
                    image_aug_batch = normalize_meanstd(image_aug_batch, axis=(1, 2))
                    ...
                    feed_dict = {image_ph: image_aug_batch,
                                 mask_ph: mask_aug_batch,
                                 training: True}
                    cost, _, _ = sess.run(
                        [loss, opt, iou_update], feed_dict=feed_dict)
                    train_iou = sess.run(iou, feed_dict=feed_dict)
                    ...
                    if train_step % 5 == 0:
                        print("Epoch: ", epoch, "Step: ", train_step, "Cost: ",
                              cost, "IoU:", train_iou)
                        loss_summary_str, iou_summary_str = sess.run([loss_summary, iou_summary], feed_dict=feed_dict)
                        train_writer.add_summary(loss_summary_str, train_step)

                        path_1_handle.writelines(str(train_step) + ';' + str(cost) + '\n')
                        path_3_handle.writelines(str(train_step) + ';' + str(train_iou) + '\n')
                        print('train_loss/iou written')
                    ...
                    if train_step % 200 == 0:
                        ...
                        print("Step: ", train_step, "val_Cost: ",
                              eval_cost, "eval_IoU:", eval_iou)

                        summary_sva = sess.run(loss_summary, feed_dict=feed_dict)
                        validation_writer.add_summary(summary_sva, train_step)
                        path_2_handle.writelines(str(train_step) + ';' + str(eval_cost) + '\n')
                        print('val_loss written')
                        # if eval_cost < validation_cost:
                        #     validation_cost = eval_cost
                        #     print("saving model with step: " + str(step))
                        #     saver.save(sess, save_dir, step)
                        #     step += 1
            path_1_handle.close()
            path_2_handle.close()
            path_3_handle.close()
    ...
    def infer(self, image_dir, batch_size, ckpt, output_folder):
        ...
        inference_file_path = '/data/Packaging_detection/Raw/infer_depth_merged.txt'
        infer_handle = open(inference_file_path, 'r+')

        files = infer_handle.readlines()
        ...
        with tf.Session() as sess:
            ...
            for file in files:
                ...
                depthmap, labels = CsvReader.load(file, img_size=(800, 800))
                image = np.expand_dims(np.expand_dims(depthmap, axis=0), axis=-1)
                
                image = normalize_meanstd(image, axis=(1, 2))
                feed_dict = {
                    image_ph: image,
                    training: True
                }
                ...
                print('inference...')
                save_path = os.path.join(os.path.dirname(file), 'prediction.png')
                cv2.imwrite(save_path, 255 * prediction)
